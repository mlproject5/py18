...
def time():
    ...
    st.markdown(
        "<center><h1 style='font-family: Comic Sans MS; font-weight: 600; font-size: 36px;'>WorldWide Time "
        "Checker</h1></center>",
        unsafe_allow_html=True)

    input_tz = st.selectbox("Select input time zone:", time_zones)
    output_tz = st.selectbox("Select output time zone:", time_zones)

    input_dt = st.date_input("Select date:")
    input_time = st.time_input("Select time:")

    input_dt_str = input_dt.strftime("%Y-%m-%d")
    input_time_str = input_time.strftime("%I:%M:%S %p")
    input_datetime_str = f"{input_dt_str} {input_time_str}"
    input_datetime = datetime.strptime(input_datetime_str, "%Y-%m-%d %I:%M:%S %p")
    input_datetime = pytz.timezone(input_tz).localize(input_datetime)
    output_datetime = input_datetime.astimezone(pytz.timezone(output_tz))

    st.markdown(
        "<h1 style='font-family: Comic Sans MS; font-weight: 600; font-size: 24px;'>Output "
        "</h1></center>",
        unsafe_allow_html=True)
    st.info(
        f"Input Datetime: **{input_datetime.strftime('%I:%M:%S %p')}** {input_datetime.strftime('%Y-%m-%d')} {input_tz}")
    st.warning(
        f"Output datetime: **{output_datetime.strftime('%I:%M:%S %p')}** {output_datetime.strftime('%Y-%m-%d')} {output_tz}")
